...
const buttonClasses = computed(() => ({
  ...
  'pl-16':
    props.icon && props.label && props.buttonType !== 'ghost' && props.iconPosition === 'left',
  'pr-16':
    props.icon && props.label && props.buttonType !== 'ghost' && props.iconPosition === 'right',
}))

const isLink = computed(() => !!props.href)
const sanitizedUrl = computed(() => sanitizeUrl(props.href))

const renderIcon = () => (props.icon ? h(props.icon, { class: 'ds-button-icon' }) : undefined)
